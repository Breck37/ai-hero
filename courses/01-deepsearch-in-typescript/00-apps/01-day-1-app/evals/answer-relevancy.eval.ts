...
// Simple in-memory cache for answer relevancy checks
const relevancyCache = new Map<string, any>();
...
export const checkAnswerRelevancy = async (opts: {
  ...
  const cacheKey = `${opts.question}:${opts.submission}`;

  // Check cache first
  if (relevancyCache.has(cacheKey)) {
    console.log("Using cached answer relevancy result");
    return relevancyCache.get(cacheKey);
  }
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      // Step 1: Generate statements from the submission
      ...
      if (statements.length === 0) {
        ...
        relevancyCache.set(cacheKey, result);
        return result;
      }
      ...
      console.warn(`Answer relevancy check attempt ${attempt} failed:`, error);
  ...
  console.error("All answer relevancy check attempts failed:", lastError);
  const fallbackResult = {
    score: 0.5, // Neutral score as fallback
    metadata: {
      statements: [],
      verdicts: [],
      rationale: `Failed to evaluate answer relevancy after ${maxRetries} attempts. Last error: ${lastError?.message || "Unknown error"}`,
    },
  };

  // Cache the fallback result too
  relevancyCache.set(cacheKey, fallbackResult);
